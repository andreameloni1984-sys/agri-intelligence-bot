...
def fetch_table(url):
    r = requests.get(url, timeout=20, headers={"User-Agent":"AgriIntelligenceBot/1.0"})
    r.raise_for_status()
    soup = BeautifulSoup(r.text, "html.parser")
    rows = []
    for tr in soup.select("table tr"):
        cells = [c.get_text(" ", strip=True) for c in tr.select("th,td")]
        if len(cells) >= 5 and cells[0].lower() not in ("piazza", "market"):
            rows.append(cells)
    return rows
...
def parse_ovicaprini():
    ...
    for c in rows:
        ...
        if price is not None:
            out.append({"market":c[0], "date":c[1], "product":c[2],
                        "price":price, "unit":"€/kg", "weekly_change":change,
                        "url":URL_OVICAPRINI})
    return out
...
def parse_lattiero_caseari():
    ...
    for c in rows:
        ...
        change = _num(c[4])
        if price is not None:
            unit = "€/hl" if "latte di pecora" in c[2].lower() else "€/kg"
            out.append({"market":c[0], "date":c[1], "product":c[2],
                        "price":price, "unit":unit, "weekly_change":change,
                        "url":URL_LATTIERO})
    return out
